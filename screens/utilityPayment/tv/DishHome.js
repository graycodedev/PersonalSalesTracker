...
export class DishHome extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      customerid: "",
  ...
  GetDetails = async () => {
    ...
    var response = await (await request())
      .get(api.DishHome.UserLoopUp + "?casid=" + this.state.customerid)
    ...
    if (response != undefined) {
      if (response.data.Code == 200) {
        this.props.navigation.navigate("DishHome Payment", {
...
export class DishHomePayment extends React.Component {
  constructor(props) {
    super(props);
    ...
    this.state = {
      ...
      customeraddress: data.data.customer_address,
      package: data.data.package,
      quanity: data.data.quantity,
  ...
  confirmpayment = async () => {
    ...
    var confirmationData = [
      ...
      },
      {
        ...
        value: this.state.customeraddress.split(",")[0],
      },
      {
        label: "Package :",
    ...
    var response = await (await request())
      .post(api.DishHome.MakePayment, data)
      .catch(function(error) {
        ToastMessage.Short("Error Ocurred Contact Support");
      });
    if (response != undefined) {
          ...
          {this.state.showpin == false ? (
            <View style={{zIndex:99}}>
              ...
              <View style={styles.detailcontainer}>
                ...
                <View style={styles.rowdata}>
                  <Text style={styles.variablename}>Customer Address:</Text>
                  <Text style={styles.valuestyle}>
                    {this.state.customeraddress.split(",")[0]}
                  </Text>
                </View>
                <View style={styles.rowdata}>
                  <Text style={styles.variablename}>Package:</Text>
                  <Text style={styles.valuestyle}>{this.state.package}</Text>
                </View>
                <View style={styles.rowdata}>
                  <Text style={styles.variablename}>Quanity:</Text>
                  <Text style={styles.valuestyle}>{this.state.quanity}</Text>
                </View>
              </View>
...
export class DishHomeSuccessful extends React.Component {
  constructor(props) {
    super(props);
  }
  render() {
