...
export class SagarmathaInsurance extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      debitNoteNumberError: "",
      transactionId: "",
      debitNoteNo: "001618/198500779",
      note: "",
      noteError: "",
      isFavourite: false,
      spinner: false,
  ...
  validateUser = () => {
    var isValid = true;
    if (this.state.debitNoteNo == "") {
      this.setState(() => ({
        debitNoteNumberError: "Enter the debit note number",
      }));
      isValid = false;
    }
  ...
  GetDetails = async () => {
    let model = {
      debitNoteNo: this.state.debitNoteNo,
    ...
    if (response != undefined) {
      if (response.data.Code == 200) {
        this.props.navigation.navigate("Sagarmatha Payment", {
          data: response.data.Data.Data,
          model: model,
        });
      } else {
        ToastMessage.Short("Invalid Debit Note Number");
      }
    } else {
      ToastMessage.Short("Error Ocurred Contact Support");
    }
  ...
  render() {
    return (
      ...
      >
        ...
        >
          <Text style={styles.bold_text}>Debit Note Number</Text>
          <TextInput
            style={{
              width: "100%",
              height: 45,
            ...
            onChangeText={(text) => {
              this.setState({ debitNoteNo: text });
            }}
          />
          {this.state.debitNoteNumberError != "" && (
...
export class SagarmathaInsurancePayment extends React.Component {
  constructor(props) {
    super(props);
  ...
  confirmPayment = async () => {
    ...
    if (response != undefined) {
      if (response.data.Code == 200) {
        ...
        this.props.navigation.navigate("Sagarmatha Success", {
          ...
          {this.state.showpin == false ? (
            <View style={{zIndex: 99}}>
              ...
              <View style={styles.detailcontainer}>
                ...
                <View style={styles.rowdata}>
                  <Text style={styles.variablename}>Debit Note No :</Text>
                  <Text style={styles.value}>{this.state.debitNoteNo}</Text>
                </View>
                <View style={styles.rowdata}>
                  <Text style={styles.variablename}>Address :</Text>
                  <Text style={styles.value}>{this.state.address}</Text>
                </View>
                <View style={styles.rowdata}>
                  <Text style={styles.variablename}>Contact Number:</Text>
...
export class SagarmathaInsuranceSuccessful extends React.Component {
  constructor(props) {
    super(props);
  }
  render() {
