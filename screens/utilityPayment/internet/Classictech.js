...
export class Classictech extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      transactionId: "",
  ...
  GetDetails = async () => {
    ...
    var response = await (await request())
      .get(
        api.Internet.GetDetails +
          ...
          "&InternetName=ClassictechInternet"
      )
    ...
    if (response != undefined) {
      if (response.data.Code == 200) {
        this.props.navigation.navigate("Classic Tech Payment", {
...
export class ClassictechPayment extends React.Component {
  constructor(props) {
    super(props);
  ...
  choosepackage = (item) => {
    ...
    this.setState({ packagename: item.package });
          ...
          <View style={{ marginBottom: 15 }}>
            {this.state.plans.map((item, index) => {
              return (
                ...
                >
                  <Text>
                    {item.package + " - " + item.duration + " months"}
                  </Text>
                </TouchableOpacity>
              );
            })}
  ...
  confirmpayment = async () => {
    ...
    if (await this.checkBalance()) {
      ...
      if (response != undefined) {
        if (response.data.Code == 200) {
          this.props.navigation.navigate("Classic Tech Successful", {
...
export class ClassictechSuccessful extends React.Component {
  constructor(props) {
    super(props);
  }
  render() {
