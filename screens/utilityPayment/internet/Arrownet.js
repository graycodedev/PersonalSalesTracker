...
export class Arrownet extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      transactionId: "",
  ...
  GetDetails = async () => {
    ...
    var response = await (await request())
      .get(
        api.Internet.GetDetails +
          "?uniqueId=" +
          this.state.transactionId +
          "&UserName=" +
          this.state.customerid +
          "&InternetName=ArrownetInternet"
      )
    ...
    if (response != undefined) {
      if (response.data.Code == 200) {
        this.props.navigation.navigate("Arrownet Payment", {
...
export class ArrownetPayment extends React.Component {
  constructor(props) {
    super(props);
  ...
  validateForm() {
    ...
    if (this.state.amount == null) {
      isvalid = false;
      this.setState(() => ({
        packageNameError: "Select one of the packages !",
      }));
    } else {
  ...
  choosepackage = (item) => {
    this.setState({ month: item.duration });
      ...
      <View>
        ...
        >
          {this.state.month != null ? (
            <Text>
              {this.state.month + " months - Rs. " + this.state.amount}
            </Text>
          ) : (
          ...
          <View style={{ marginBottom: 15 }}>
            {this.state.plans.map((item, index) => {
              return (
                ...
                >
                  <Text>{item.info + " - Rs. " + item.amount}</Text>
                </TouchableOpacity>
              );
            })}
          </View>
  ...
  confirmpayment = async () => {
    ...
    if (await this.checkBalance()) {
      ...
      if (response != undefined) {
        if (response.data.Code == 200) {
          this.props.navigation.navigate("Arrownet Successful", {
          ...
          {this.state.showpin == false ? (
              ...
              <View style={styles.detailcontainer}>
                <View style={styles.rowdata}>
                  ...
                  <Text style={styles.valuestyle}>{this.state.fullname}</Text>
                </View>
                ...
                <Text style={styles.variablename}>Duration:</Text>
                <View
                  style={{
                    justifyContent: "center",
                    marginTop: 10,
                ...
                >
                  {this.state.month != null && (
                    <Text>{this.state.month} months</Text>
                  )}
                </View>
              </View>
...
export class ArrownetSuccessful extends React.Component {
  constructor(props) {
    super(props);
  }
  render() {
