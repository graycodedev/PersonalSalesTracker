...
const OrderDetails = ({ navigation, route }) => {
    ...
    const getDetail = async () => {
        console.log('del', orderId)
        var response = await (await request())
          .get(Api.Orders.Details + "?id=" + orderId)
    ...
    const deliverOrder = async (id) => {
      console.log("dfd", Api.Deliver.Save + "?id=" + orderId)
      var response = await (await request())
        .post(Api.Deliver.Save , qs.stringify({id: orderId}))
        .catch(function (error) {
        ...
        >
            {!isLoading && orderDetail? 
                <><View style={styles.container}>
                   ...
                   {orderDetail?.OrderDate && <View>
                        <RegularInputText
                            key="deliverydate"
                            placeholder="Date"
                            value={(new Date(orderDetail.OrderDate)).toLocaleDateString("en-UK", {
                                year: "numeric",
                                month: "2-digit",
                                day: "2-digit",
                            })}
                ...
                <View>
                   ...
                   {orderDetail?.EstimatedDeliveryDate && <View>
                        <RegularInputText
                            key="deliverydate"
                            placeholder="Date"
                            value={(new Date(orderDetail.EstimatedDeliveryDate)).toLocaleDateString("en-UK", {
                                year: "numeric",
                                month: "2-digit",
                                day: "2-digit",
                            })}
                ...
                >
                  ...
                  >
                    ...
                    <Text style={{ fontFamily: "Regular", fontSize: 16 }}> {orderDetail.PartyOutputVM.PartyName}</Text>
                  </View>
                </View>
                </View>

   {orderDetail?.OrderDetailOutputVM  &&
                <View style={styles.ordersView}>
                    <Text style={{fontFamily:"SemiBold", fontSize: 16, marginBottom: 4}}>Products</Text>
                    <View>
                        {orderDetail?.OrderDetailOutputVM.map((product, index) => (
                ...
                {orderDetail?.OrderDetailOutputVM  && <View style={{borderTopWidth: 1, borderBottomWidth:1, borderColor:"black", padding: 8,marginVertical: 8}}>
                    <View style={{flexDirection:"row"}}> 
                    <Text style={{fontFamily:"SemiBold", fontSize: 16}}>Qty:</Text>
                    
                    <Text style={{fontFamily:"SemiBold", fontSize: 16, marginLeft: 4}}>{orderDetail?.OrderDetailOutputVM.reduce((sum, item) => sum + item.SoldQuantity, 0)}</Text>
                    </View>
                    <View style={{flexDirection:"row"}}> 
                    <Text style={{fontFamily:"SemiBold", fontSize: 16}}>Total Amount:</Text>
                    
                    <Text style={{fontFamily:"SemiBold", fontSize: 16, marginLeft: 4}}>{orderDetail?.OrderDetailOutputVM.reduce((sum, item) => sum + item.SoldQuantity* item.Rate, 0).toFixed(2)}</Text>
                    </View>
...
{orderDetail.IsDispatched && !orderDetail.IsDelivered && !orderDetail.IsCanceled &&  <TouchableOpacity  onPress={()=>{
