        ...
        {!requestDetails ? (
          ...
          <View style={styles.container}>
            <View style={{ alignSelf: "flex-end" }}>
              {requestDetails?.IsCancelled ? (
                <Text style={{ color: "orange" }}>Cancelled</Text>
              ) : requestDetails?.ApprovedOrRejected == "A" ? (
                <Text style={{ color: "green" }}>Approved</Text>
              ) : requestDetails?.ApprovedOrRejected == "R" ? (
                <Text style={{ color: "red" }}>Rejected</Text>
              ) : (
                <Text style={{ color: "orange" }}>Requested</Text>
              )}
            </View>
            <Text style={styles.noteHead}>{requestDetails.ItemName}</Text>
            <View style={styles.noteView}>
              <Text style={styles.noteText}>
                {requestDetails.RequestRemarks}
              </Text>
            </View>
            <View style={styles.noteView}>
              <Text style={styles.noteText} numberOfLines={4}>
                Requested On: <DateDisplay date={requestDetails?.RequestedOn} />
              </Text>
            </View>
          </View>
        )}
