...
const ReturnOrder = (props) => {
  ...
  useEffect(() => {
    ...
    if (update) {
      setReturnReason({
        value: returnItem?.OrderReturnReasonId,
        label: returnItem?.ReturnReasonTitle,
      });
      setProductName({
        value: returnItem?.ProductId,
        label: returnItem?.ProductName,
      });
    }
    fetchReturnReasons();
    fetchProductNames();
  ...
  return (
    ...
    >
        ...
        <View style={{ marginBottom: 15, zIndex: 98 }}>
          ...
          >
            <Text style={{ fontFamily: "Regular", fontSize: 14 }}>
              ...
              {!selectedParty
                ? update
                  ? returnItem?.PartyName ?? "No Party Name"
                  : "Add Party"
      ...
      {toastMessage.length > 0 && (
        <Toast message={toastMessage} isVisible={toastMessage.length > 0} />
      )}
    </ScrollView>
  );
};
